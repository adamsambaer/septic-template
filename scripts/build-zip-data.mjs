...
for (const [key, zips] of [...shards].sort()) {
  ...
  const shard = {
    // zip → [lat, lng, town, state]. The coordinates place the ZIP for the
    // distance sort; the town and state are the client's own address, which
    // Sapt requires on the site-settings item and which the form would
    // otherwise have to ask for separately.
    z: Object.fromEntries(zips.map((z) => [z.zip, [round(z.la), round(z.lo), z.place, z.state]])),
...
// Written here, not by hand: the build clears the directory first, so a
// hand-placed README disappears on the next regenerate.
fs.writeFileSync(path.join(OUT, 'README.md'), `# Town lookup data
...
- \`t\`: every town within ${RADIUS} miles of any ZIP in the shard, as
